...
function onPlay() {
    FPS = getFileOjbectVideoFrameRate();
    //createCanvasElement(document.getElementById("videoHolder")
    //    , document.getElementById("myVideo")
    //    , getCanvasId()
    //    , getCanvasClassName());

    initCanvasElement(document.getElementById("videoHolder")
        , document.getElementById("myVideo")
        , getCanvasId());
}
...
function updateVideoCurrentTimeCode() {
	...
	//if (drawAnchorRectangle(SMPTE_time)) {
    //    console.log("anchor:" + SMPTE_time);
    //}
	//if (drawAnchorRectangleByFrameNo(secondsToFrameNo(video.currentTime, FPS))) {
	//    console.log("anchor:" + SMPTE_time);
    //}

	if (drawAnchorRectangleByFrameNoOnCanvas(secondsToFrameNo(video.currentTime, FPS))) {
        console.log("anchor:" + SMPTE_time);
    }
...
function secondsToFrameNo(time, fps) {

    return  Math.floor((time* fps).toFixed(3));
}
...
function secondsToMilliseconds(time) {
    ...
    var minutes = Math.floor(time / 60) % 60;
    var seconds = Math.floor(time % 60);
    var frames = Math.floor(((time % 1) * 100).toFixed(3));
